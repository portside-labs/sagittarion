...
describe('ConnectionManager', () => {
  it('opens a SQLite connection end to end and reports it', async () => {
    ...
    expect(catalog.schemas).toEqual([{ name: 'main', counts: { table: 5, view: 1, function: 0, index: expect.any(Number), trigger: 1 } }])
    const page = await driver.listObjects({ kinds: ['table', 'view'] })
    expect(page.items.map((o) => o.name).sort()).toEqual(['big_numbers', 'order_summary', 'orders', 'settings', 'users', 'weird name'])
    expect(page.cursor).toBeNull()
    const found = await driver.searchObjects('ord')
    expect(found.objects.map((o) => `${o.kind}:${o.name}`)).toEqual(expect.arrayContaining(['table:orders', 'view:order_summary', 'index:idx_orders_user', 'trigger:orders_touch_user']))
    expect((await driver.definition({ kind: 'trigger', name: 'orders_touch_user' })).sql).toMatch(/CREATE TRIGGER/i)
    expect((await driver.tablesMeta([{ name: 'users' }])).map((m) => m.name)).toEqual(['users'])
